...
namespace injector
{
    __int32 GetProcessId(const char* processname)
    {
        ...
        HANDLE Snapshot = li(CreateToolhelp32Snapshot)(TH32CS_SNAPPROCESS, 0);
        if (!li(Process32First)(Snapshot, &Entry)) {
            return 0;
        }

        while (li(Process32Next)(Snapshot, &Entry)) {

            if (strcmp((const char*)Entry.szExeFile, processname) == 0) {
    ...
    public:
        ...
        ManualMap(__int8* imagebuffer, const char* processname)
        {
            ...
            while (ImportDescriptor->Name) {
                ...
                HMODULE ModuleBaseAddress = li(LoadLibraryA)(ModuleName);
                ...
                while (OriginalFirstThunkData->u1.AddressOfData) {
                    ...
                    if (IMAGE_SNAP_BY_ORDINAL(OriginalFirstThunkData->u1.Ordinal)) {
                        ...
                        FunctionAddress = (__int64)(li(GetProcAddress)(ModuleBaseAddress, FunctionName));
                    }
                        ...
                        FunctionAddress = (__int64)li(GetProcAddress)(ModuleBaseAddress, ImportByName->Name);
                    }
            ...
            void* RemoteAllocBase = li(VirtualAllocEx)(ProcessHandle, (void*)imagebuffer, SizeOfImage, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
            if (!RemoteAllocBase) {

                RemoteAllocBase = li(VirtualAllocEx)(ProcessHandle, nullptr, SizeOfImage, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
                if (!RemoteAllocBase) {
            ...
            li(WriteProcessMemory)(ProcessHandle, RemoteAllocBase, Allocbase, SizeOfImage, &BytesWritten)) 
            
            li(CreateRemoteThread)(ProcessHandle, 0, 0, (LPTHREAD_START_ROUTINE)((__int64)RemoteAllocBase + NtHeaders->OptionalHeader.AddressOfEntryPoint), 0, 0, 0);
        }
    };
}