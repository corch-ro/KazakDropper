...
#define xorstr(str) ::AngelEncryption::xor_string([]() { return str; }, std::integral_constant<std::size_t, sizeof(str) / sizeof(*str)>{}, std::make_index_sequence<::AngelEncryption::stuff::_buffer_size<sizeof(str)>()>{})
...
namespace AngelEncryption {
    ...
    public:
        ...
        XORSTR_FORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
            : _storage{ ::AngelEncryption::stuff::load_from_reg((std::integral_constant<std::uint64_t, stuff::load_xored_str8<Size>(Keys, Indices, l())>::value))... }
        {}
        ...
        {
            alignas(alignment) std::uint64_t keys[]{ ::AngelEncryption::stuff::load_from_reg(Keys)... };
        ...
        XORSTR_FORCEINLINE pointer crypt_get() noexcept
        {
            ...
            alignas(alignment)
                std::uint64_t arr[]{ ::AngelEncryption::stuff::load_from_reg(Keys)... };
            std::uint64_t* keys =
                (std::uint64_t*)::AngelEncryption::stuff::load_from_reg((std::uint64_t)arr);
#else
            alignas(alignment) std::uint64_t keys[]{ ::AngelEncryption::stuff::load_from_reg(Keys)... };
#endif
    ...
    xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>)->xor_string<
        ...
        Size,
        std::integer_sequence<std::uint64_t, stuff::key8<Indices>()...>,
